...
bool ExhibitionItem::create(Person *p_person) {
...
bool ExhibitionItem::read(std::string person_id) {
...
bool ExhibitionItem::update(std::string person_id) {
  std::string path = "library_db/" + person_id + ".data";
  std::ofstream file(path);
  if (!file.is_open()) return false;
  file << "id=" << this->id << "\n";
...
bool ExhibitionItem::remove(std::string person_id) {
  std::string path = "library_db/" + person_id + ".data";
